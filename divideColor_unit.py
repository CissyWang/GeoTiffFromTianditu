...
src = cv2.imread(r"4.png")  # 这里填你的原图像路径
cv2.namedWindow("input", cv2.WINDOW_AUTOSIZE)
cv2.imshow("input", src)


hsv = cv2.cvtColor(src, cv2.COLOR_BGR2HSV)  # BGR转HSV
mask = cv2.inRange(hsv, lowerb=np.array([0, 80, 80]), upperb=np.array([130, 255, 255]))  # 提取紫色h=0~30、黄、橙道路，主要道路
# cv2.imshow("image", mask)

gray = cv2.cvtColor(src, cv2.COLOR_BGR2GRAY)
ret, gray = cv2.threshold(gray, 249, 255, cv2.THRESH_BINARY_INV)  # 提取次要道路
print(ret)
...
# mask = cv2.medianBlur(mask, 3) #核尺寸是奇数
# 黑色背景转透明部分
